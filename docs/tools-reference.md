...
- When the client advertises a model to sample from (and `SEMANTIC_NAMING_MODE` is not `review`), the response also pre-fills a `suggestions` dict via the host LLM, ready to pass straight to `apply_semantic_names`
...
**Confirmation.** A client that can render an elicitation *form* is asked to confirm first, with a checkbox the user has to tick. A client offering only URL elicitation is not asked, because it cannot render the form. The approval names one connection: if the session connects to a different database while the question is open, or while the cleanup waits its turn for that connection's writer lock, nothing is deleted and the response says so. Declining, dismissing the question, or accepting without ticking the box all cancel the cleanup. On MCP 2026-07-28 the question travels as a multi round-trip request (the tool returns it and is called again with the answer); on earlier revisions the tool waits for the answer. The question is asked before the connection's writer lock is taken, so a slow answer holds nobody up. A client that cannot be asked is not asked, and the cleanup proceeds as it always did.
